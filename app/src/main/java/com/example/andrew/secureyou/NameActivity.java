...
public class NameActivity extends AppCompatActivity {
    ...
    //to get the users real time

    public void genarateCode(View v) {
        Date myDate = new Date();
        SimpleDateFormat format1 = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss a", Locale.getDefault());
        String date = format1.format(myDate);
        Random r = new Random();

        //time to display the map on the users screen 1 second
    ...
    //the user to select their image for recognision purpose with their unique code
